...
from strategy import MACDExceedStrategy
...
def main(opt):
    ...
    calculator = MACDExceedStrategy()
    trade, data = calculator(data)
    indicator = StockIndicator(trade, start_date, end_date, stock_type='Stock')
    indicator.show()
    addp = [mpf.make_addplot(data['macdhist'], type='bar', panel=2, secondary_y=False)]
