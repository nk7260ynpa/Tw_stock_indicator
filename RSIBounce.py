...
def main(opt):
    ...
    indicator.show()
    addp = [mpf.make_addplot(data['RSI'], panel=2, secondary_y=False),
            mpf.make_addplot([80]*len(data['RSI']), panel=2, secondary_y=False),
            mpf.make_addplot([40]*len(data['RSI']), panel=2, secondary_y=False)]
