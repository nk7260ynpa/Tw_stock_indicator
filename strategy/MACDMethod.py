...
class MACDExceedStrategy(BaseStrategy):
    """
    ...
    def __call__(self, data):
        data = data.join(MACD(data, fastperiod=12, slowperiod=26, signalperiod=9))
        for i in range(data.shape[0]-1):
            ...
            if self.order_day is None and c_macd > 0:
                self.order_day = n_time
                self.order_price = n_open
            
            elif self.order_day is not None and c_macd < -0.005:
